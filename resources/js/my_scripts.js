...
function loadStatsPage(){
	...
	var rowLength = stats_table.rows.length;
	var win_count=0;
	var lose_count=0;
	for ( var i =2; i< rowLength; i++){
		var home_points = parseInt(stats_table.rows[i].cells[2].innerHTML);
		var opp_points = parseInt(stats_table.rows[i].cells[3].innerHTML);
		console.log(home_points,opp_points)
		if (home_points>opp_points){
			stats_table.rows[i].cells[4].innerHTML="CU Boulder"
			win_count+=1
		}
		else{
			stats_table.rows[i].cells[4].innerHTML=stats_table.rows[i].cells[1].innerHTML
			lose_count+=1
		}
	}
	...
	Football Player Information Page
		...
		function loadPlayersPage()
		{
			...
			for(var i=0; i<4; i++)
			{
				...
				string_i=i.toString(10);
				aTag.setAttribute("onClick", "switchPlayers('"+string_i+"')")
				console.log(aTag)
